    ...
    useEffect(() => {
        const fetchUserData = async () => {
            try {
                ...
                if (userData) {
                    // User found, fetch projects related to user_id
                    ...
                    // If not in User table, check in College table
                    const { data: collegeData, error: collegeError } = await supabase
                        .from('College')
                        .select('id, college_name')
                        .eq('college_name', username)
                        .single();
                    ...
                    if (collegeData) {
                        ...
                        // Handle case where neither User nor College found with the given username
                        console.error('User or College not found with the given username');
                    }
                }
            } catch (error: any) {
    ...
    <ul className="list-disc pl-6">
        {projects.map((project) => (
            <li key={project.id} className="mb-6">
                ...
                <p className="text-lg">Links: {project.content}</p>
            </li>
        ))}
    </ul>
</motion.div>
